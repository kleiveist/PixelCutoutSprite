    ...
    pub fn parse(value: impl Into<String>) -> Result<Self, DomainError> {
        ...
        if !valid {
            return Err(DomainError::invalid(
                "slot_id",
                "must be 1–64 lowercase ASCII letters, digits, or internal underscores",
            ));
        }
        Ok(Self(value))
    }
...
impl ActionKey {
    pub fn parse(value: impl Into<String>) -> Result<Self, DomainError> {
        ...
        if !valid {
            return Err(DomainError::invalid(
                "action_key",
                "must start with a lowercase ASCII letter and contain only letters, digits, or underscores",
            ));
        }
        Ok(Self(value))
    }
