...
- [Unreleased — Vault, Cutout and Sprite workflow](RELEASE-NOTES.md)
- [Security Policy](SECURITY.md)
- [PixelCutoutSprite Studio — Einstieg](START_HERE.md)
<!-- PYGINDEX:README END -->

PixelCutoutSprite Studio is an offline-first Tauri desktop application. React provides the
shared studio shell; Rust protects local vault files, sessions, locks and recovery.

The application combines three vault-backed workflows:

- Prompt: one vault base profile, nine asset categories, a paginated questionnaire and automatic Markdown outputs.
- Cutout: independent overlapping pixel masks, local assisted selection, exact PNG crops and a position manifest.
- Sprite: automatic assembly from that manifest, layer order/visibility/locks, transforms and non-destructive scene autosave.

The old project/Area/Motion/Dummy/Outfit/NPC application and its legacy export commands have
been removed. Existing vault files are preserved; opening them does not migrate or delete old
projects. There is no animation timeline, cloud image generation or game-engine export.

Start and validate through the existing repository entry point:
...
[Complete German workflow](docs/guides/gesamtworkflow.md) ·
[Cutout guide](docs/guides/cutout-willkommen.md) ·
[Sprite guide](docs/guides/sprite-studio.md) ·
[Prompt guide](docs/guides/prompt-generator.md) ·
[P37 removal and preservation evidence](docs/developer/acceptance/P37-cutout_altbasis_entfernen_willkommen.md).

P00–P27 reports describe the historical predecessor. Lichterhain generation, Godot export,
app-data Prompt writes and Area handoff are no longer product functions. Old source and test
evidence remains recoverable in Git. The [P43 acceptance report](docs/developer/acceptance/P43-gesamtabnahme_haertung_dokumentation.md)
separates actual native results, browser fixtures and open platform gates. Windows/macOS and
Windows Explorer new-window behavior require their own native evidence; a Linux build is not
a cross-platform release approval.

Unsigned native test candidates still use the unchanged build tooling, for example:
