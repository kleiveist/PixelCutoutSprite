...
def test_desktop_toolchain_versions_are_exact_and_cross_language_aligned() -> None:
    ...
    assert frontend["dependencies"]["@tauri-apps/plugin-dialog"] == (
        support.tauri_dialog
    )
    assert (
        frontend_lock["packages"][""]["dependencies"]["@tauri-apps/plugin-dialog"]
        == support.tauri_dialog
    )
...
def test_studio_ci_builds_and_starts_each_unsigned_native_candidate() -> None:
    ...
    assert (
        "cargo test --locked --manifest-path src-tauri/Cargo.toml --lib p37_tests"
        in workflow
    )
    assert "--test recovery_integrity" in workflow
    assert "src/shared/vault/VaultWelcome.test.tsx" in workflow
    assert "src/shared/vault/vault-client.test.ts" in workflow
    ...
    assert not re.search(
        r"\bgh\s+release\b|\bgit\s+push\b|\btauri\s+signer\b", workflow
    )
